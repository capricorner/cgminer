 ...
 *  ChangeLog :
 *      
 *      2014-04-01  simoncc     Add 'libbitfury_sendHashData_n' for multi-chip devices 'NFx'
 *
 */
			...
			for (i = 0; i < 16; i++) {
				if (oldbuf[i] != newbuf[i] && info->owork[chip_n]) {
					...
					if (bitfury_checkresults(thr, info->owork[chip_n], nonce))
					    applog(LOG_DEBUG, "CHIP %d Solved!!", chip_n);
						info->nonces++;
				}
			}
...
// simoncc  2014-04-01  modified from 'libbitfury_sendHashData', and this funciton is for multi-chip
bool libbitfury_sendHashData_n(struct thr_info *thr, struct cgpu_info *bitfury, struct bitfury_info *info)
{
	unsigned newbuf[17];
	unsigned *oldbuf;
	struct bitfury_payload *p;
	unsigned int localvec[20];
	int x;
    ...
    for (x=0;x<info->chips;x++) {
        oldbuf = &info->oldbuf[17 * x];
    	memcpy(newbuf, info->spibuf + 4 + (20*4) * x, 17 * 4);
    
    	info->job_switched[x] = newbuf[16] != oldbuf[16];
    
    	if (likely(info->second_run[x])) {
    		if (info->job_switched[x]) {
    			int i;
    
    			for (i = 0; i < 16; i++) {
    				if (oldbuf[i] != newbuf[i] && info->owork[x]) {
    					uint32_t nonce; //possible nonce
    
    					nonce = decnonce(newbuf[i]);
    					if (bitfury_checkresults(thr, info->owork[x], nonce)) {
    					    applog(LOG_DEBUG, "CHIP %d found nonce!!", x);
    						info->nonces++;
    					}
    				}
    			}
    		...
    		info->second_run[x] = true;
    }
	cgsleep_ms(BITFURY_REFRESH_DELAY / info->chips * 5);
