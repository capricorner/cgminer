 ...
 *  ChangeLog :
 *      
 *      2014-04-01  simoncc     1. Add 'bitfury_check_work_n' for multi-chip devices 'NFx'
 *                              2. Modify 'nf1_detect_one to find the number of chips and freq
 *                              3. Modify 'nf1_reinit' for multi-chip
 *
 */
...
// simoncc  2014-04-01  modified for multi-chip initialization
static bool nf1_reinit(struct cgpu_info *bitfury, struct bitfury_info *info)
{
    ...
    for (i=0;i<info->chips;i++) {
    	...
    	applog(LOG_DEBUG, "CHIP %d INITED", i);
	}
...
static bool nf1_detect_one(struct cgpu_info *bitfury, struct bitfury_info *info)
{
	...
	// simoncc  2014-04-01
    //simoncc ------begin---
    //finding out "NFx-Y" or "NFx/Y"; x for chips and Y is A(50) to F(55)
    pNF = strstr(bitfury->usbdev->prod_string, "NF");
    if (pNF) {
        applog(LOG_DEBUG, "NFx product string ==> %s", bitfury->usbdev->prod_string);
        memset(buf, 0, 10);
        memcpy(buf, pNF+2, 4);
        ...
        info->chips = atoi(buf); //get the number
        
        if (opt_nf1_bits != 50) {
            // no "--nfu-bits", so continue parsing product string to find the freq letter...
            switch (*pNF) {                                    
            case 'a':
            case 'A':
            default:
        ...
        //default if no NFx ID string
        info->chips = 1; 
    }
    applog(LOG_DEBUG, "NFx ==> chip : %d   Freq : %d", info->chips, opt_nf1_bits);
    //simoncc -----end---
...
// simoncc  2014-04-01  modified from 'bitfury_check_work', and this funciton is for multi-chip
static void bitfury_check_work_n(struct thr_info *thr, struct cgpu_info *bitfury, struct bitfury_info *info)
{
    int i;
    for (i=0;i<info->chips;i++) {
    	if (!info->work[i]) {
    		info->work[i] = get_work(thr, thr->id);
    		if (unlikely(thr->work_restart)) {
    			free_work(info->work[i]);
    			info->work[i] = NULL;
    			return; //.... simoncc ... maybe lack of something to free... TODO
    		} 
    		bitfury_work_to_payload(&info->payload[i], info->work[i]);
    	}
    }
	...
	if (!libbitfury_sendHashData_n(thr, bitfury, info))
		usb_nodev(bitfury);
		
    for (i=0;i<info->chips;i++) {
    	if (info->job_switched[i]) {
    		if (likely(info->owork[i]))
    			free_work(info->owork[i]);
    		info->owork[i] = info->work[i];
    		info->work[i] = NULL;
    	}
    }
}
...
{
	int64_t ret = 0;
    // simoncc  2014-04-01  changed to multi-chip funtion...
	bitfury_check_work_n(thr, bitfury, info);
